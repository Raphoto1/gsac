...
export default function ContentCardsSection({ title, items, intro }: ContentCardsSectionProps) {
  return (
    <section className='relative overflow-hidden bg-base-200 px-4 py-16 md:px-8 md:py-20'>
      <div className='mx-auto flex w-full max-w-6xl flex-col gap-10'>
        ...
        <div className='grid gap-5 md:grid-cols-2 xl:grid-cols-3'>
          {items.map((item) => (
            <article
              key={item.key}
              className='rounded-3xl border border-base-300 bg-base-100 px-6 py-7 shadow-[0_18px_45px_rgba(15,23,42,0.08)] transition-transform duration-300 hover:-translate-y-1'
            >

...
import type { HomeProductIcon } from "@/types/home-products";
...
function chunkProducts(items: ProductItem[], size: number): ProductItem[][] {
...
type ProductsNetworkClusterProps = {
  cluster: ProductItem[];
  groupIndex: number;
  particlesActive: boolean;
};

function ProductsNetworkCluster({ cluster, groupIndex, particlesActive }: ProductsNetworkClusterProps) {
  ...
  const idSuffix = `-${groupIndex}`;

  return (
    <div className={groupIndex === 0 ? "" : "mt-18 md:mt-24"}>
      <div className="relative md:hidden">
        <div className="pointer-events-none absolute inset-0" aria-hidden="true">
          <svg viewBox="0 0 360 1200" className="h-full w-full overflow-visible">
            ...
            <path id={`product-mobile-a${idSuffix}`} d="M95 115 C165 150, 225 185, 270 310" fill="none" stroke="transparent" />
            <path id={`product-mobile-b${idSuffix}`} d="M270 310 C235 415, 160 470, 120 600" fill="none" stroke="transparent" />
            <path id={`product-mobile-c${idSuffix}`} d="M120 600 C175 700, 235 760, 275 845" fill="none" stroke="transparent" />
          </svg>
        </div>

        <div className="relative z-10 flex flex-col gap-20 px-3 py-6">
          {cluster.map((product, index) => (
            <div key={`${product.title}-${groupIndex}-${index}`} className={`w-[88%] max-w-72 ${mobilePositions[index] ?? "self-start"}`}>
              <CardBasicProp
                title={product.title}
      ...
      <div className="relative hidden md:block">
        <div className="pointer-events-none absolute inset-0 hidden md:block xl:hidden" aria-hidden="true">
          <svg viewBox="0 0 1000 980" className="h-full w-full overflow-visible">
            ...
            <path id={`product-grid-md-a${idSuffix}`} d="M220 155 C380 118, 515 120, 760 135" fill="none" stroke="transparent" />
            <path id={`product-grid-md-b${idSuffix}`} d="M220 155 C245 345, 285 585, 320 850" fill="none" stroke="transparent" />
            <path id={`product-grid-md-c${idSuffix}`} d="M760 135 C640 235, 565 290, 500 325" fill="none" stroke="transparent" />
          </svg>
        </div>
          ...
          <svg viewBox="0 0 1200 780" className="h-full w-full overflow-visible">
            ...
            {particlesActive ? (
              <>
                <circle r="5" className="company-network-dot-svg">
                  <animateMotion dur="4.8s" repeatCount="indefinite" rotate="auto">
                    <mpath href={`#product-grid-xl-a${idSuffix}`} />
                  </animateMotion>
                </circle>
                <circle r="5" className="company-network-dot-svg delay-a">
                  <animateMotion dur="5.2s" repeatCount="indefinite" rotate="auto">
                    <mpath href={`#product-grid-xl-b${idSuffix}`} />
                  </animateMotion>
                </circle>
                <circle r="5" className="company-network-dot-svg delay-b">
                  <animateMotion dur="5.6s" repeatCount="indefinite" rotate="auto">
                    <mpath href={`#product-grid-xl-c${idSuffix}`} />
                  </animateMotion>
                </circle>
              </>
            ) : (
            ...
            <path id={`product-grid-xl-a${idSuffix}`} d="M200 140 C345 105, 460 105, 600 145" fill="none" stroke="transparent" />
            <path id={`product-grid-xl-b${idSuffix}`} d="M600 250 C520 430, 430 595, 330 710" fill="none" stroke="transparent" />
            <path id={`product-grid-xl-c${idSuffix}`} d="M600 250 C690 425, 775 580, 870 705" fill="none" stroke="transparent" />
          </svg>
        </div>
        ...
        <div className="relative z-10 hidden md:block md:h-272 xl:h-192">
          {cluster.map((product, index) => (
            <div
              key={`${product.title}-${groupIndex}-${index}`}
...
export default function ProductsList({
  ...
  sectionTitle = "Soluciones conectadas",
  sectionDescription = "Una red de servicios y productos pensada para operar en conjunto, con el mismo lenguaje visual del ecosistema de empresas.",
}: ProductsListProps) {
  const [particlesActive, setParticlesActive] = useState(true);
  const displayProducts = products.slice(0, 10);
  const productGroups = chunkProducts(displayProducts, 5);
    ...
    <section className="relative w-full overflow-hidden bg-base-100 px-4 py-16">
      ...
      <div className="mx-auto w-full max-w-6xl">
        <div className="relative z-10">
          <p className="text-center text-sm font-semibold uppercase tracking-[0.18em] text-primary">{sectionLabel}</p>
          <div className="mt-3">
            <CompanyListHeader
              title={sectionTitle}
        ...
        <div className="relative z-10 mt-14">
          {productGroups.map((cluster, groupIndex) => (
            <ProductsNetworkCluster
              key={`product-cluster-${groupIndex}`}
              cluster={cluster}

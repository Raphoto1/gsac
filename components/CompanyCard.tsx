...
export default function CompanyCard({
  ...
  return (
    <article className="company-card-float group relative mx-auto flex w-full max-w-[18rem] flex-col items-center rounded-4xl border border-base-300 bg-base-100 px-5 pb-5 pt-18 text-center shadow-[0_16px_40px_rgba(15,23,42,0.08)] transition-transform duration-300 hover:-translate-y-2 hover:shadow-[0_24px_56px_rgba(15,23,42,0.14)]">
      ...
      <figure className="absolute left-1/2 top-0 flex h-20 w-[10.5rem] -translate-x-1/2 -translate-y-1/2 items-center justify-center overflow-hidden rounded-[1.75rem] border border-base-300 bg-base-100 px-4 py-3 shadow-xl">
        {hasLogo ? (
          <Image
            src={logo}
            alt={name}
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        {relationship ? (
          <div className="mt-4 w-full rounded-2xl border border-base-300 bg-base-200/60 px-3 py-2 text-left">
            <p className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-primary/80">{relationshipLabel}</p>
            <p className="mt-1 text-sm leading-relaxed text-base-content/80">{relationship}</p>
          </div>
        ) : null}
